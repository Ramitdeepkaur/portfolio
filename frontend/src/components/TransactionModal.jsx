...
const TransactionModal = ({ isOpen, onClose, onSave, holdings = [], initialTransaction = null }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 px-4">
      <div className="w-full max-w-xl rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl shadow-black/40">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            <h3 className="text-lg font-semibold text-slate-100">{initialTransaction ? 'Edit transaction' : 'Add transaction'}</h3>
            <p className="text-sm text-slate-400">Record a manual cash movement for a holding.</p>
          </div>
          <button onClick={onClose} className="rounded-full border border-slate-700 p-2 text-slate-400 hover:text-slate-100">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              <label className="mb-1 block text-sm text-slate-300">Holding</label>
              <select
                value={form.holding}
              ...
              {errors.holding && <p className="mt-1 text-xs text-rose-400">{errors.holding}</p>}
            </div>
          ...
          <div className="rounded-xl border border-emerald-900/40 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-300">

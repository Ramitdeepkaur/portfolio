  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
        ...
        <p className="mt-1 text-sm text-slate-400">Track portfolio changes with before/after snapshots and actor metadata.</p>
      </div>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
        ...
        <div className="mt-4 space-y-3">
          {filteredLogs.length === 0 ? (
            <div className="rounded-xl border border-dashed border-slate-700 px-4 py-8 text-center text-sm text-slate-400">No audit events found.</div>
          ) : (
              ...
              <div key={log.id} className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="rounded-full border border-slate-700 px-2.5 py-1 text-xs uppercase tracking-wide text-slate-300">{log.action}</span>
                      <span className="text-sm font-semibold text-slate-100">{log.entity}</span>
                    </div>
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2">
                  <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-3">
                    ...
                    <pre className="mt-2 whitespace-pre-wrap text-sm text-slate-300">{log.before}</pre>
                  </div>
                  <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-3">
                    <p className="text-xs uppercase tracking-wider text-slate-500">After</p>
                    <pre className="mt-2 whitespace-pre-wrap text-sm text-slate-300">{log.after}</pre>
                  </div>
                </div>
              </div>
            ))

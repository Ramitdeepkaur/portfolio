      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">Save what-if plans, forecasts, and retirement simulations for the current portfolio.</p>
          </div>
      ...
      <div className="grid gap-4">
        {scenarios.length === 0 ? (
          ...
          scenarios.map((scenario) => (
            <div key={scenario.id} className="rounded-2xl border border-slate-800 bg-slate-950/70 p-5">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-sm text-slate-400">{scenario.description || 'No description provided.'}</p>
                </div>
                <div className="flex flex-wrap gap-2">
                  <span className="rounded-full border border-slate-700 px-2.5 py-1 text-xs uppercase tracking-wide text-slate-300">{scenario.scenarioType}</span>
                  <span className="rounded-full border border-emerald-800/40 bg-emerald-500/10 px-2.5 py-1 text-xs text-emerald-300">Base value: ${Number(scenario.basePortfolioValue || portfolioValue).toLocaleString()}</span>
                </div>
              </div>
